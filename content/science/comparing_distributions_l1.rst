...
the expectancy differs most.
The specific choice of class of function
defines the metric. If we now consider a kernel, it implicitly defines a
space of
functions (intuitively related to all the possible KDEs generated by
varying data points): a Reproducible Kernel Hilbert Space (RKHS). When F is the
unit ball in such an RKHS, the corresponding metric, the IPM, is known as
the Maximum Mean Discrepancy (MMD). It can be shown that, rather than
computing the maximum, the MMD is equal to the RKHS distance between the
mean embeddings:
